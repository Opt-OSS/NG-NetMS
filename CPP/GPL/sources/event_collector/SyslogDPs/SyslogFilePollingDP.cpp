...
SyslogFilePollingDP::SyslogFilePollingDP(string FileName,
...
void SyslogFilePollingDP::OnReadLine(const std::string& Line) {
	...
	if (repeatFound) {
		for (size_t i = 0; i < repetitionCount; ++i) {
			DataProviderListener::DataProviderEvent event(
					DataProviderListener::DataProviderEvent::Event::DATA,
					m_PreviousLine);
			m_Notifier.Notify(event);
		}
	} else {
		DataProviderListener::DataProviderEvent event(
				DataProviderListener::DataProviderEvent::Event::DATA, Line);
		m_Notifier.Notify(event);
		m_PreviousLine = Line;
	}
}
