...
FilePollReader::FilePollReader(): m_Handlers(nullptr), m_InotifyFd(0), m_WatchDescriptor(0)
{
}
...
bool FilePollReader::Run()
{
	...
	for (;;)
	{
		...
		for (char *p = buffer.get(); p < buffer.get() + numRead;)
		{
			...
			// Process events
			if (event->mask & (IN_DELETE | IN_MOVE_SELF | IN_MOVED_FROM | IN_DELETE_SELF))
			{
			...
			if (event->mask & IN_MODIFY)
			{
				if (!ReadData())
				{
					...
					{
						m_Logger->LogDebug(
							" File size " + std::to_string(stat_buf.st_size) + " less than current stream position " +
							std::to_string(bytes_read) + ",possibly truncated, try to reopen file"
						);
						m_File.close();
						return false;
					}
