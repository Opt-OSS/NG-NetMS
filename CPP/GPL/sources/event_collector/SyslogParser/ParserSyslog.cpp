...
vector<string> TokenParser::m_Months = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
...
{
public:
	TimestampRFC3164Parser(): m_Month(1), m_Day(0), m_Hours(0), m_Minutes(0), m_Seconds(0) {}
...
public:
	CiscoDummyCounterParser(): m_Counter(0) {}
...
{
public:
	CiscoTimestampParser(): m_YearExist(false), m_Year(0), m_Month(1), m_Day(0), m_Hours(0), m_Minutes(0), m_Seconds(0) {}
	...
	bool IsYearPresent() { return m_YearExist; }
	...
	string& GetTimeshift() { return m_Timeshift; }
...
map<string, int> CiscoTimestampParser::m_CiscoTimezones = {
	{"ACT", 95},   {"ADT", 30},	  {"AET", 100},	  {"AEST", 100}, {"AGT", -30},	 {"AHST", -100}, {"ART", 20},  {"AST", -90},
	{"AT", -20},   {"BET", -30},  {"BST", 10},	  {"BT", 30},	 {"CAT", 10},	 {"CCT", 80},	 {"CDT", -50}, {"CEST", 20},
	{"CET", 10},   {"CNT", -35},  {"CST", -60},	  {"CTT", 80},	 {"EADT", -110}, {"EAST", 100},	 {"EAT", 30},  {"ECT", 10},
	{"EDT", -40},  {"EET", 20},	  {"EST", -50},	  {"FST", -20},	 {"FWT", 10},	 {"GMT", 0},	 {"GST", 100}, {"HDT", 90},
	{"HST", -100}, {"IDLE", 120}, {"IDLW", -120}, {"IET", -50},	 {"IST", 55},	 {"JST", 90},	 {"MDT", -60}, {"MEST", -20},
	{"MESZ", -20}, {"MET", 10},	  {"MEWT", 10},	  {"MIT", -110}, {"MST", -70},	 {"MYT", 80},	 {"NET", 40},  {"NST", 120},
	{"NT", -110},  {"NZDT", 130}, {"NZST", 120},  {"NZT", 120},	 {"PDT", -70},	 {"PLT", 50},	 {"PNT", -70}, {"PRT", -40},
	{"PST", -80},  {"SST", 110},  {"SWT", 10},	  {"UTC", 0},	 {"VST", 70},	 {"WADT", -80},	 {"WAST", 70}, {"WAT", -10},
	{"YDT", -80},  {"YST", -90},  {"ZP4", 40},	  {"ZP5", 50},	 {"ZP6", 50}};

// Format 2017-05-19 00:05:47+00
class UnknownTimestampParser : public TokenParser
{
public:
	UnknownTimestampParser(): m_Year(0), m_Month(1), m_Day(0), m_Hours(0), m_Minutes(0), m_Seconds(0), m_Timezone(0) {}
...
public:
	...
	string& GetSubFacility() { return m_SubFacility; }
...
public:
	...
	string& GetFacility() { return m_Facvility; }
...
public:
	bool Parse(string Input)
	{
		...
		if ((string::npos == processPid.find('[') || string::npos == processPid.find(']')) && !kernel)
		{
			return false;
		}
		...
		if (kernel)
		{
			...
			if (secondPart.size() && string::npos == processPid.find('[') && string::npos == processPid.find(']'))
			{
...
public:
	...
	string& GetPriority() { return m_Priorioty; }
...
{
public:
	JunosStructuredTimestampParser(): m_Year(0), m_Month(0), m_Day(0), m_Hours(0), m_Minutes(0), m_Seconds(0) {}
	...
	string& GetMilliseconds() { return m_Milliseconds; }
...
map<int, string> NetscreenFacilityParser::m_CodeToFacility = {{257, "traffic"}, {528, "SSH"}, {536, "IKE"}, {518, "ADM"}};
...
bool ParserSyslog::Parse(string Message, bool HasSourceIp, string SourceIP)
{
	...
	if (netscreen)
	{
		...
		Event event(
			EventProtocol::SYSLOG, priority, GetTimestamp(), GetTimestamp(timestampRFC3164Parser), hostName, facility, eventCode, Message, OriginalMessage, 0, 0
		);
	...
	else
	{
		...
		if (junosFormat)  // We sure that we parse JunOS structured message
		{
			...
			Event event(
				EventProtocol::SYSLOG, eventPriority, GetTimestamp(), GetTimestamp(timestampParser), hostName, facility, eventCode, Message, OriginalMessage, 0, 0
			);
			m_Notifier.Notify(event);
			return true;
		}
		else  // CISCO | Linux PC message
		{
			...
			Event event(
				EventProtocol::SYSLOG, eventPriority, GetTimestamp(), eventOriginalTimestamp, hostName, eventFacility, eventCode, Message, OriginalMessage, 0, eventSeverity
			);
			m_Notifier.Notify(event);
			return true;
		}
