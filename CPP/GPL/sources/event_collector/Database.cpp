...
Database::Database(bool Debug): m_Debug(Debug), m_TerminateThreads(false), m_CondVarPredicate(false)
{
...
bool Database::CreateTables()
{
	...
	{
		string query =
			"INSERT INTO event_protocol (id, name) VALUES(" + to_string(static_cast<int>(e.first)) + ", '" + e.second + "');";
		PerformQuery(query);
	}
...
void Database::WritterThread(Database* This)
{
	while (!This->m_TerminateThreads)
	{
		...
		string baseQuery = This->m_Debug ? "INSERT INTO events ( priority, severity, origin_ts, receiver_ts, origin, "
										   "origin_id, facility, code, descr, protocol, raw_event ) VALUES "
										 : "INSERT INTO events ( priority, severity, origin_ts, receiver_ts, origin, "
										   "origin_id, facility, code, descr, protocol ) VALUES ";
...
DbReturnCode Database::WriteEvent(const Event& event)
{
	...
	{
		auto predicate = [](const pair<string, pair<time_t, int> >& val)
		{
			return val.second.second == -1;
		};
	...
	string queryInsertEvent = " ( '" + event.getPriority() + "', '" + to_string(severity) + "', '" + event.getOrign_Ts() +
							  "', '" + event.getTs() + "', '" + event.getOrigin() + "', '" + to_string(RouterId) + "', '" +
							  event.getFacility() + "', '" + event.getCode() + "', '" + EscapeString(event.getDescr()) +
