...
bool Database::Connect(const DbSettings& Settings)
{
	...
	{
		m_Connector = std::make_shared<DbConnector>(Settings);
	}
...
DbReturnCode Database::CreateTables(const vector<int>& Intervals)
{
	...
	for (int interval : Intervals)
	{
		...
		string createTableT2 =
			"CREATE TABLE IF NOT EXISTS observer_history_t2_" + IntervalToString(interval) +
...
DbReturnCode Database::ProfileLastInterval(time_t Time, int Interval)
{
	...
	string profileT1 =
		"INSERT INTO observer_history_t1_" + IntervalToString(Interval) +
		" (value,observer_options_id,ts) SELECT MAX( value ) AS value, MAX( observer_options_id) AS observer_options_id, '" +
		MakeTimeStamp(avg_time) + "' AS ts FROM observer_history_t1 WHERE ts BETWEEN '" + MakeTimeStamp(start_time) +
		"' AND '" + MakeTimeStamp(end_time) + "' GROUP BY( observer_options_id );";

	string profileT2 =
		"INSERT INTO observer_history_t2_" + IntervalToString(Interval) +
		" (value,observer_options_id,ts) SELECT MAX( value ) AS value, MAX( observer_options_id) AS observer_options_id, '" +
		MakeTimeStamp(avg_time) + "' AS ts FROM observer_history_t2 WHERE ts BETWEEN '" + MakeTimeStamp(start_time) +
		"' AND '" + MakeTimeStamp(end_time) + "' GROUP BY( observer_options_id );";
...
DbReturnCode Database::ProfileAllData(int Interval)
{
	...
	string procedureT1 = "CREATE OR REPLACE FUNCTION profile_oprionsT1( integer, text ) RETURNS SETOF void AS $$ "
						 "declare "
						 "timestamps TIMESTAMP WITH TIME ZONE[] := ARRAY(SELECT to_timestamp( floor( ( extract('epoch' from ts "
						 ") / $1 ) ) * $1 ) AS interval_alias FROM observer_history_t1  GROUP BY interval_alias); "
						 "start_time TIMESTAMP WITH TIME ZONE; "
						 "end_time TIMESTAMP WITH TIME ZONE; "
						 "avg_time TIMESTAMP WITH TIME ZONE; "
						 "BEGIN "
						 "EXECUTE 'CREATE TABLE IF NOT EXISTS '|| quote_ident( $2 ) ||' ( value BIGINT, observer_options_id "
						 "INTEGER NOT NULL, ts TIMESTAMP WITH TIME ZONE NOT NULL )'; "
						 "DROP TABLE IF EXISTS prf_tmp; "
						 "CREATE TEMP TABLE IF NOT EXISTS prf_tmp ( value BIGINT, observer_options_id INTEGER NOT NULL, ts "
	...
	string procedureT2 = "CREATE OR REPLACE FUNCTION profile_oprionsT2( integer, text ) RETURNS SETOF void AS $$ "
						 "declare "
						 "timestamps TIMESTAMP WITH TIME ZONE[] := ARRAY(SELECT to_timestamp( floor( ( extract('epoch' from ts "
						 ") / $1 ) ) * $1 ) AS interval_alias FROM observer_history_t2 GROUP BY interval_alias); "
						 "start_time TIMESTAMP WITH TIME ZONE; "
						 "end_time TIMESTAMP WITH TIME ZONE; "
						 "avg_time TIMESTAMP WITH TIME ZONE; "
						 "BEGIN "
						 "EXECUTE 'CREATE TABLE IF NOT EXISTS '|| quote_ident( $2 ) ||' ( value double precision, "
						 "observer_options_id INTEGER NOT NULL, ts TIMESTAMP WITH TIME ZONE NOT NULL )'; "
						 "DROP TABLE IF EXISTS prf_tmp; "
						 "CREATE TEMP TABLE IF NOT EXISTS prf_tmp ( value double precision, observer_options_id INTEGER NOT "
						 "NULL, ts TIMESTAMP WITH TIME ZONE NOT NULL ); "
						 "FOREACH end_time IN ARRAY timestamps "
						 "LOOP "
						 "avg_time   = to_timestamp( extract('epoch' from end_time ) - ($1 / 2)); "
						 "start_time = to_timestamp( extract('epoch' from end_time ) - ($1 - 1)); "
