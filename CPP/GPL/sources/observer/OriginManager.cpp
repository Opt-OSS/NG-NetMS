...
OriginManager::OriginManager(shared_ptr<Database>& db, shared_ptr<LogWriter>& logger, int imax):
	m_Database(db), m_Logger(logger), m_MaxInterval(imax)
{
}
...
void originThread(Origin& origin, Profiler& profiler, int imax)
{
	...
	while (!origin.TestOptions(newOptTypes, mtxOptTypes))
	{
		...
		threadLog(
			"Can not get data from origin(" + myIp + "). The next try in " + to_string(interval) + "s. max " + to_string(imax),
			origin.GetId()
		);
	...
	try
	{
		...
		if (m_Database->ReadOriginModels(models))
		{
			...
			for (auto aRow : models)
			{
				...
				m_Logger->LogInfo(
					"Current origin model settings are: " + to_string(m._id) + " Vendor: " + m._vendor + " Model: " + m._model +
					" Version: " + m._snmpVersion
				);
				...
				if (m_Database->ReadOriginModelOptions(pqOriginModelOptions, m._id))
				{
					...
					for (auto aRow : pqOriginModelOptions)
					{
						...
						m_Logger->LogInfo(
							"Current origin model options are: " + to_string(o._id) + " ModelID: " + to_string(o._modelId) +
							" Type: " + to_string(o._type) + " Name: " + o._name + " OID: " + o._oid + " Unit: " + o._unit
						);
					}
				}
				else
