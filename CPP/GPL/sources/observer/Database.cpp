...
Database::Database(shared_ptr<LogWriter>& logger, bool mdef): m_Logger(logger), m_MDef(mdef)
{
}
...
bool Database::CreateHistoryTable(const SnmpVT& t)
{
	...
	m_Query += "do $$ BEGIN "
			   " IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'lnk_observer_options_" +
			   tableName +
			   "') THEN "
			   " ALTER TABLE public." +
			   tableName +
			   " "
			   " ADD CONSTRAINT lnk_observer_options_" +
			   tableName +
			   " "
	...
	m_Query += " DO $$ BEGIN "
			   ...
			   " WHERE  c.relname = '" +
			   tableName +
			   "_observer_options_id' "
			   " AND    n.nspname = 'public' "
			   " ) THEN "
			   " CREATE INDEX " +
			   tableName + "_observer_options_id ON public." + tableName +
			   " USING btree( observer_options_id ASC NULLS LAST ); "
			   " END IF; "
			   " END; $$;";
	...
	m_Query += " DO $$ BEGIN "
			   ...
			   " ) THEN "
			   " CREATE INDEX " +
			   tableName + "_ts ON public." + tableName +
			   " USING btree( ts ASC NULLS LAST );"
			   " END IF; "
			   " END; $$;";
...
bool Database::SetModelOption(ModelOption& opt)
{
	...
	m_Query += "VALUES ( '" + opt._name + "', '" + opt._oid + "', '" + opt._unit + "', " + std::to_string(opt._modelId) + ");";
...
bool Database::AddModel(Model& model)
{
	...
	m_Query += "VALUES ( '" + model._model + "', '" + model._vendor + "', '" + model._snmpVersion + "');";
...
bool Database::AddObserverOption(SnmpOption& opt)
{
	...
	m_Query += "VALUES ( " + to_string(opt._modelOptionId) + ", " + to_string(opt._originId) + ", " +
...
void Database::SetValue(SnmpValue& value, const time_t& ts)
{
	...
	m_Query += "INSERT INTO observer_history_t" + std::to_string(value._type) + " (observer_options_id, value, ts) VALUES (" +
...
bool Database::GetComunityStr(int oid, string& comunity)
{
	m_Query = "SELECT community_ro FROM snmp_access WHERE id = "
			  "(SELECT snmp_access_id FROM router_snmp_access WHERE router_id = " +
			  std::to_string(oid) + ");";
