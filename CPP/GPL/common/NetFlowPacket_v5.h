...
{
public:
	NetFlowPacket_v5(NetFlowHeader_v5& Header, const unsigned char* Records): m_Header(Header)
	{
	...
	bool operator==(const NetFlowPacket_v5& other) const { return m_Header == other.m_Header && m_Records == other.m_Records; }

	bool operator!=(const NetFlowPacket_v5& other) const { return !operator==(other); }

	const NetFlowHeader_v5& GetHeader() const { return m_Header; }

	size_t GetRecordCount() const { return m_Records.size(); }

	const std::list<NetFlowRecord_v5>& GetRecords() { return m_Records; }
