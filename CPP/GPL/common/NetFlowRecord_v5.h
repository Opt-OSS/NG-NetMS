...
{
public:
	NetFlowRecord_v5() { memset(&m_record, 0, sizeof(record)); }

	NetFlowRecord_v5(const unsigned char* Header) { memcpy(&m_record, Header, sizeof(record)); }

	NetFlowRecord_v5(const NetFlowRecord_v5& other) { m_record = other.m_record; }

	static size_t GetRecordSize() { return sizeof(record); }
	...
	bool operator==(const NetFlowRecord_v5& other) const { return memcmp(&m_record, &other.m_record, sizeof(record)); }

	bool operator!=(const NetFlowRecord_v5& other) const { return 0 != memcmp(&m_record, &other.m_record, sizeof(record)); }

	uint32_t GetSrcAddr() const { return invertEndianess(m_record.srcaddr); }

	uint32_t GetDstAddr() const { return invertEndianess(m_record.dstaddr); }

	uint32_t GetNextHop() const { return invertEndianess(m_record.nexthop); }

	uint16_t GetInput() const { return invertEndianess(m_record.input); }

	uint16_t GetOutput() const { return invertEndianess(m_record.output); }

	uint32_t GetDPkts() const { return invertEndianess(m_record.dPkts); }

	uint32_t GetDOctets() const { return invertEndianess(m_record.dOctets); }

	uint32_t GetFirst() const { return invertEndianess(m_record.first); }

	uint32_t GetLast() const { return invertEndianess(m_record.last); }

	uint16_t GetSrcPort() const { return invertEndianess(m_record.srcport); }

	uint16_t GetDstPort() const { return invertEndianess(m_record.dstport); }

	uint8_t GetTcpFlags() const { return m_record.tcp_flags; }

	uint8_t GetProt() const { return m_record.prot; }

	uint8_t GetTOS() const { return m_record.tos; }

	uint16_t GetSrcAS() const { return invertEndianess(m_record.src_as); }

	uint16_t GetDstAS() const { return invertEndianess(m_record.dst_as); }

	uint8_t GetSrcMask() const { return m_record.src_mask; }

	uint8_t GetDstMask() const { return m_record.dst_mask; }
