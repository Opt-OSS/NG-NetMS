...
string Cryptography::DatabaseEncrypt(string Key, string Text)
{
	...
	try
	{
		...
		string Cipher;
		StringSource(
			Text, true, new StreamTransformationFilter(e, new Base64Encoder(new StringSink(Cipher)), BlockPaddingSchemeDef::NO_PADDING)
		);
		return Cipher;
	}
	...
	try
	{
		...
		CryptoPP::StringSource decryptor(
			...
			new CryptoPP::Base64Decoder(
				new CryptoPP::StreamTransformationFilter(d, new CryptoPP::StringSink(Recovered), BlockPaddingSchemeDef::NO_PADDING)
			)
		);
