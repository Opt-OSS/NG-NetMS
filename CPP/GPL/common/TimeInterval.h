...
public:
	TimeInterval(const TimeInterval& other): m_Value(other.m_Value), m_Units(other.m_Units) {}

	TimeInterval(): m_Value(0), m_Units(Units::Seconds) {}

	TimeInterval(int Value, Units Unit): m_Value(Value), m_Units(Unit) {}

	void SetValue(int Value) { m_Value = Value; }

	void SetUnit(Units Value) { m_Units = Value; }

	int GetValue() const { return m_Value; }

	Units GetUnit() const { return m_Units; }
	...
	bool operator==(const TimeInterval& other) const { return m_Value == other.m_Value && m_Units == other.m_Units; }

	bool operator!=(const TimeInterval& other) const { return m_Value != other.m_Value || m_Units != other.m_Units; }

	bool operator<(const TimeInterval& other) { return GetValueInseconds() < other.GetValueInseconds(); }
