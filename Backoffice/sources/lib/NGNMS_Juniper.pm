...
use NGNMS_DB;
use NGNMS_util;
use Net::Juniper qw($Error $debug $TIMEOUT);


# $Net::Juniper::debug=1;

if (defined($ENV{"NGNMS_TIMEOUT"})) {
  $Net::Juniper::TIMEOUT = $ENV{"NGNMS_TIMEOUT"};
} else {
  $Net::Juniper::TIMEOUT = 60;
}
...
sub juniper_create_session {
  ...
  my @passwds = @_[2..3];
  $Error = undef;
  $session = Net::Juniper->new();
  defined($session) or
    ( $Error = "Failed to log in to $host" and return);

  $session->open($host, $username, @passwds);
...
sub juniper_get_topologies ($$$$) {
  ...
  juniper_get_file('show isis database extensive', $host."_isis.txt") or
    return $Error;

  print "Getting OSPF topology...\n";
  juniper_get_file('show ospf database extensive', $host."_ospf.txt")  or
    return $Error;
