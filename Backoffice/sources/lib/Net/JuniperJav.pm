...
##jav sub host		{ $_[0]->opened ? $_[0]->_socket->host : undef }
sub host		{
...
## jav sub opened		{ $_[0]->_socket && $_[0]->_socket->opened }
sub opened		{
					 if($self->_access eq 'Telnet')
...
##			passphrase => $passphrase,
 ...
 ##   return $flag ? $self->open($access,$host, $username, @passwords) : $self;
 return $self;
}
...
sub open {
    ...
    if($access eq 'Telnet')
	{
		...
		my ($intro, $match);
		for (;;) {
		($intro, $match) = $self->_socket->waitfor(Timeout => 60,
