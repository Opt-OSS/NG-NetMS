...
describe 'CREATE CMD::MASSCAN' => sub {
        my $verbose = 1;
        my $scanner;
        before each => sub {
                $scanner = NGNMS::SubnetScanner->new({ workdir => $TMP_DIR, verbose => $verbose, scan_engine => 'masscan' });
            };
        it 'use masscan' => sub {
                like $scanner->create_cmd(), qr#masscan#;
            };
        it 'should add --offline if dryrun' => sub {
                $scanner->{dryrun} = 1;
                like $scanner->create_cmd(), qr/--offline/, '--offline if dryrun';
                $scanner->{dryrun} = 0;
                unlike $scanner->create_cmd(), qr/--offline/, 'no offline if no dryrun';
            };
...
describe 'PREPARE :: ' => sub {
        ...
        my $mock_output_file = $Fixtures_dir.'/mock_console_output_massscan';
        ...
        before each => sub {
                ...
                $scanner = NGNMS::SubnetScanner->new({ DB => $DB, verbose => $verbose, workdir => $TMP_DIR, scan_engine =>
                    'masscan' });
            };
        xit 'writes to db while scannig' => sub {
                ...
                $scanner->execute_scan($fh);
                ok 1;
            };
        it 'prepares (get host,sort, uniq) raw output' => sub {

                my $in_filename = 'result_raw.txt';
                my $out_filename = 'result_prep.txt';
                #        Emsgd::diag( \@expect);
                copy $Fixtures_dir.'/mass_scan_result', $TMP_DIR.'/'.$in_filename or fail  'Bad fixtures';

                ok $scanner->prepare_result($in_filename, $out_filename);

                my @expect = read_file $Fixtures_dir.'/masscan_prepared_result', chomp => 1;
