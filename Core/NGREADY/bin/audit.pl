...
if (defined $ls_discovery) {
    if ($force_rediscovery) {
        ...
        if ($ls_discovery > 600)  ## potential issue here if one discovery not updated longer than 10min
        {
...
#Emsgd::diag \%bgp_host_ips ,'bgp_host_ips hostname=>ip';
#Emsgd::diag \%bgp_links ,'bgp_links';
#Emsgd::diag \%bgp_autonomous_systems , 'bgp_autonomous_systems';
...
sub discoveryBgp  {
    ...
    $logger->debug ('About to audit hosts:',
    ...
    foreach my $seedHost (@$arr_bgps) {
        ...
        if (!$test_topologies) {
            ...
            if (!$hostType) {
                ...
                $logger->debug( "Using $hostType from DN for $seedHost") if $hostType;
            }
        ...
        $logger->info( "$seedHost: host type \"$hostType\"");
...
Usage: audit.pl [switches] host user passwd enpasswd accesstype[Telnet/SSHv1/SSHv2]
    ...
    -H        passphrase for public key
    -i        interactive status updates
    --force-rediscovery Force new discovery. That is - even if previouse process still runningnew process will be started
    Example:
