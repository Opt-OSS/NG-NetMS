...
sub schedule{
    ...
    my $seedHost = $self->DB->decode_val_from_DB('seedHost');
    ...
    my $file_content = $self->compose_file($scanner,$seedHost);
...
env \$(cat $ENV{NGNMS_HOME}/env.list | xargs) /usr/bin/perl  $ENV{NGNMS_HOME}/bin/audit.pl  $debug_flag  $scan_flag $seedHost
END_MESSAGE
