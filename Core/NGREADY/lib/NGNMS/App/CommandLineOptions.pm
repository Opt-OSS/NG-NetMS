...
option "record" => (
        ...
        negativable => 1,
        doc         =>
        "record [default=1] results of executed on remote host macros into  direcory --record-dir, file per macro, disable record with --no-record option"
    );
option "record_dir" => (
        is      => 'ro',
        format  => 's',
        ...
        doc     =>
        "record output of executed on remote host macros into given direcory, file per macro, default : \$ENV{'NGNMS_HOME'}/data/rtconfig/{Router_id}"
    );
option "play" => (
        is     => 'ro',
        format => 's',
...
option "host_timeout" => (
        is     => 'ro',
        format => 'i',
        doc    => "Timeout to connect to the remote host"
    );
...
option "inject" => (
        ...
        negativable => 1,
        doc         =>
        "Inject router in DB on PollHost mode if router  not exsists, used  only with  --host-type and --play"
    );
1;
